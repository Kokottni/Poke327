...
typedef struct character{
  ...
  int nextTurn;
	int seq_num;
  character_type_t symbol;
	pc_t *pc; //exactly one pc in game
	npc_t *npc; //points to unique npc
} character_t;

character_t characterMap[MAP_Y][MAP_X];
  ...
  for (y = 0; y < MAP_Y; y++) {
    for (x = 0; x < MAP_X; x++) {
      if (world.pc.pos[dim_y] == y &&
        ...
        switch(characterMap[y][x].symbol){
          case char_hiker:
            putchar('h');
            break;
          case char_rival:
...
static int32_t character_cmp(const void *key, const void *with){
  if((characterMap[((character_t *) key)->npc->pos[dim_y]][((character_t *) key)->npc->pos[dim_x]].nextTurn == 
      characterMap[((character_t *) with)->npc->pos[dim_y]][((character_t *) with)->npc->pos[dim_x]].nextTurn)){
      return (characterMap[((character_t *) key)->npc->pos[dim_y]][((character_t *) key)->npc->pos[dim_x]].seq_num - 
              characterMap[((character_t *) with)->npc->pos[dim_y]][((character_t *) with)->npc->pos[dim_x]].seq_num);
     }else{
      return (characterMap[((character_t *) key)->npc->pos[dim_y]][((character_t *) key)->npc->pos[dim_x]].nextTurn - 
              characterMap[((character_t *) with)->npc->pos[dim_y]][((character_t *) with)->npc->pos[dim_x]].nextTurn);
     }
}
...
static void moveCharacters(map_t *m, int hm[21][80], int rm[21][80], int numT){
  heap_t h;
  heap_init(&h, character_cmp, NULL);
  character_t *n = malloc(sizeof(character_t));
  character_t dummy;
  dummy.seq_num = 1000;
  int i, z, found;
  ...
  for(i = 0; i < numT; ++i){
    ...
    if(i == 0){
      ...
      c->symbol = 1 + rand() % (num_character_types - 2);
    }
    if(c->symbol == char_swimmer){
      int j, g;
      found = 0;
      for(j = 1; j < MAP_Y; ++j){
        for(g = 1; g < MAP_X; ++g){
          if(m->map[j][g] == ter_water){
            found = 1;
            npc->pos[dim_x] = g;
            npc->pos[dim_y] = j;
          }
      ...
      npc->pos[dim_x] = 1 + rand() % (MAP_X - 2);
      npc->pos[dim_y] = 1 + rand() % (MAP_Y - 2);
    }
    c->npc = npc;
    if(c->symbol == char_wanderer || c->symbol == char_pacer || c->symbol == char_explorer || c->symbol == char_rival){
      int j, g, lfound;
      for(j = c->npc->pos[dim_y]; j < MAP_Y; ++j){
        for(g = c->npc->pos[dim_x]; g < MAP_X; ++g){
          if((m->map[j][g] == ter_path ||  m->map[j][g] == ter_mart || m->map[j][g] == ter_center || m->map[j][g] == ter_grass || m->map[j][g] == ter_clearing) && characterMap[j][g].seq_num != 1000){
            lfound = 1;
            npc->pos[dim_x] = g;
            npc->pos[dim_y] = j;
          }
    ...
    // if(c->symbol == char_hiker){
    //   int j, g, lfound;
    //   for(j = c->npc->pos[dim_y]; j < MAP_Y; ++j){
    //     for(g = c->npc->pos[dim_x]; g < MAP_X; ++g){
    //       if((m->map[j][g] != ter_water || m->map[j][g] != ter_gate) && characterMap[j][g].seq_num != 1000){
    //         lfound = 1;
    //         npc->pos[dim_x] = g;
    //         npc->pos[dim_y] = j;
    //       }
    ...
    //   }
    // }
    npc->ter = m->map[npc->pos[dim_y]][npc->pos[dim_x]];
    npc->direction = rand() % 4;
    c->pc = NULL;
    c->seq_num = i + 1;
    c->nextTurn = 0;
    if(c->symbol == char_sentry || (c->symbol == char_swimmer && found == 0)){
      c->nextTurn = 1000000;
    }
    c->hn = heap_insert(&h, c);
    characterMap[c->npc->pos[dim_y]][c->npc->pos[dim_x]] = *c;
  }
  print_map(m);
  while((n = heap_remove_min(&h))){
    n->hn = NULL;
    int xloc, yloc, xp, yp, compCost, startLocx, startLocy;
    startLocx = n->npc->pos[dim_x];
    startLocy = n->npc->pos[dim_y];
    switch (n->symbol){
      case char_hiker:
        xloc = 100;
        yloc = 100;
        compCost = 1000000;
        for(xp = n->npc->pos[dim_x] - 1; xp < n->npc->pos[dim_x] + 2; ++xp){
          for(yp = n->npc->pos[dim_y] - 1; yp < n->npc->pos[dim_y] + 2; ++yp){
            if(xp > -1 && xp < 80 && yp > -1 && yp < 21 && compCost > hm[yp][xp] && hm[yp][xp] != 0 && (xp != n->npc->pos[dim_x] && yp != n->npc->pos[dim_y])){
              xloc = xp;
              yloc = yp;
              compCost = hm[yp][xp];
            }
        ...
        if(compCost == 1000000){
          xloc = n->npc->pos[dim_x];
          yloc = n->npc->pos[dim_y];
          n->nextTurn = 1000000;
          n->seq_num = 1000000;
        }else{
          n->npc->pos[dim_x] = xloc;
          n->npc->pos[dim_y] = yloc;
          n->nextTurn += move_cost[1][m->map[yloc][xloc]];
        } 
        if(characterMap[startLocy][startLocx].seq_num == 1000){
          characterMap[startLocy][startLocx] = dummy;
          characterMap[yloc][xloc] = *n;
        }else{
          characterMap[startLocy][startLocx] = characterMap[yloc][xloc];
          characterMap[yloc][xloc] = *n;
        }
        break;
      case char_rival:
        ...
        for(xp = n->npc->pos[dim_x] - 1; xp < n->npc->pos[dim_x] + 2; ++xp){
          for(yp = n->npc->pos[dim_y] - 1; yp < n->npc->pos[dim_y] + 2; ++yp){
            if(xp > -1 && xp < 80 && yp > -1 && yp < 21 && compCost > rm[yp][xp] && rm[yp][xp] != 0 && (xp != n->npc->pos[dim_x] && yp != n->npc->pos[dim_y])){
              xloc = xp;
              yloc = yp;
              compCost = rm[yp][xp];
            }
        ...
        if(compCost == 1000000){
          ...
          n->npc->pos[dim_x] = xloc;
          n->npc->pos[dim_y] = yloc;
          n->nextTurn += move_cost[2][m->map[yloc][xloc]];
        }
        if(characterMap[startLocy][startLocx].seq_num == 1000){
          characterMap[startLocy][startLocx] = dummy;
          characterMap[yloc][xloc] = *n;
        ...
        break;
      case char_pacer:
        switch(n->npc->direction){
          case 0:
            if(rm[n->npc->pos[dim_y]][n->npc->pos[dim_x] - 1] == INT_MAX  && rm[n->npc->pos[dim_y]][n->npc->pos[dim_x] + 1] == INT_MAX
              && rm[n->npc->pos[dim_y] - 1][n->npc->pos[dim_x]] == INT_MAX && rm[n->npc->pos[dim_y] + 1][n->npc->pos[dim_x]] == INT_MAX){
                n->nextTurn = 1000000;
                n->seq_num = 1000002;
            }
            if(rm[n->npc->pos[dim_y]][n->npc->pos[dim_x] - 1] == INT_MAX && rm[n->npc->pos[dim_y]][n->npc->pos[dim_x] + 1] != INT_MAX){
              n->npc->direction = 1;
              characterMap[n->npc->pos[dim_y]][n->npc->pos[dim_x]] = dummy;
              n->npc->pos[dim_x]++;
              n->nextTurn += move_cost[2][m->map[n->npc->pos[dim_y]][n->npc->pos[dim_x]]];
              characterMap[n->npc->pos[dim_y]][n->npc->pos[dim_x]] = *n;
            }else{
              characterMap[n->npc->pos[dim_y]][n->npc->pos[dim_x]] = dummy;
              n->npc->pos[dim_x]--;
              n->nextTurn += move_cost[2][m->map[n->npc->pos[dim_y]][n->npc->pos[dim_x]]];
          ...
          case 2:
            ...
            if(rm[n->npc->pos[dim_y] - 1][n->npc->pos[dim_x]] == INT_MAX && rm[n->npc->pos[dim_y] + 1][n->npc->pos[dim_x]] != INT_MAX){
              n->npc->direction = 3;
              characterMap[n->npc->pos[dim_y]][n->npc->pos[dim_x]] = dummy;
              n->npc->pos[dim_y]++;
              n->nextTurn += move_cost[2][m->map[n->npc->pos[dim_y]][n->npc->pos[dim_x]]];
      ...
      case char_wanderer:
        switch(n->npc->direction){
          case 0:
            ...
            else if(m->map[n->npc->pos[dim_y]][n->npc->pos[dim_x] - 1] != n->npc->ter){
              n->npc->direction = rand() % 4;
              if((n->npc->direction == 0 || n->npc->direction == 1) && m->map[n->npc->pos[dim_y]][n->npc->pos[dim_x] + 1] == n->npc->ter){
                n->npc->direction = 1;
                characterMap[n->npc->pos[dim_y]][n->npc->pos[dim_x]] = dummy;
                n->npc->pos[dim_x]--;
              }
              if(n->npc->direction == 2 && m->map[n->npc->pos[dim_y] - 1][n->npc->pos[dim_x]] == n->npc->ter){
                characterMap[n->npc->pos[dim_y]][n->npc->pos[dim_x]] = dummy;
                n->npc->pos[dim_y]--;
              }else if(m->map[n->npc->pos[dim_y] + 1][n->npc->pos[dim_x]] == n->npc->ter){
                n->npc->direction = 3;
                characterMap[n->npc->pos[dim_y]][n->npc->pos[dim_x]] = dummy;
                n->npc->pos[dim_y]++;
              }else if(n->npc->direction == 3){
                characterMap[n->npc->pos[dim_y]][n->npc->pos[dim_x]] = dummy;
                n->npc->pos[dim_y]++;
              }
              n->nextTurn += move_cost[2][m->map[n->npc->pos[dim_y]][n->npc->pos[dim_x]]];
          ...
          case 2:
            if(rm[n->npc->pos[dim_y]][n->npc->pos[dim_x] - 1] == INT_MAX  && rm[n->npc->pos[dim_y]][n->npc->pos[dim_x] + 1] == INT_MAX
              ...
              n->npc->direction = rand() % 4;
              if((n->npc->direction == 2 || n->npc->direction == 3) && m->map[n->npc->pos[dim_y] + 1][n->npc->pos[dim_x]] == n->npc->ter){
                n->npc->direction = 3;
                characterMap[n->npc->pos[dim_y]][n->npc->pos[dim_x]] = dummy;
                n->npc->pos[dim_y]++;
              }
              if(n->npc->direction == 0 && m->map[n->npc->pos[dim_y]][n->npc->pos[dim_x] - 1] == n->npc->ter){
                characterMap[n->npc->pos[dim_y]][n->npc->pos[dim_x]] = dummy;
                n->npc->pos[dim_x]--;
              }else if(m->map[n->npc->pos[dim_y]][n->npc->pos[dim_x] + 1] == n->npc->ter){
                n->npc->direction = 1;
      ...
      case char_explorer:
        switch(n->npc->direction){
          case 0:
            ...
            else if(rm[n->npc->pos[dim_y]][n->npc->pos[dim_x] - 1] == INT_MAX){
              n->npc->direction = rand() % 4;
              if(n->npc->direction == 1 && rm[n->npc->pos[dim_y]][n->npc->pos[dim_x] + 1] != INT_MAX){
                n->npc->direction = 1;
                characterMap[n->npc->pos[dim_y]][n->npc->pos[dim_x]] = dummy;
                n->npc->pos[dim_x]--;
              }
              if(n->npc->direction == 2 && rm[n->npc->pos[dim_y] - 1][n->npc->pos[dim_x]] != INT_MAX){
                characterMap[n->npc->pos[dim_y]][n->npc->pos[dim_x]] = dummy;
                n->npc->pos[dim_y]--;
              }else if(rm[n->npc->pos[dim_y] + 1][n->npc->pos[dim_x]] != INT_MAX){
                n->npc->direction = 3;
                characterMap[n->npc->pos[dim_y]][n->npc->pos[dim_x]] = dummy;
                n->npc->pos[dim_y]++;
              }else if(n->npc->direction == 3){
          ...
          case 1:
            if(rm[n->npc->pos[dim_y]][n->npc->pos[dim_x] - 1] == INT_MAX  && rm[n->npc->pos[dim_y]][n->npc->pos[dim_x] + 1] == INT_MAX
              ...
              n->npc->direction = rand() % 4;
              if((n->npc->direction == 0 || n->npc->direction == 1) && rm[n->npc->pos[dim_y]][n->npc->pos[dim_x] - 1] != INT_MAX){
                n->npc->direction = 0;
                characterMap[n->npc->pos[dim_y]][n->npc->pos[dim_x]] = dummy;
                n->npc->pos[dim_x]--;
              }
          ...
          case 2:
            if(rm[n->npc->pos[dim_y]][n->npc->pos[dim_x] - 1] == INT_MAX  && rm[n->npc->pos[dim_y]][n->npc->pos[dim_x] + 1] == INT_MAX
              ...
              n->npc->direction = rand() % 4;
              if((n->npc->direction == 2 || n->npc->direction == 3) && rm[n->npc->pos[dim_y] + 1][n->npc->pos[dim_x]] != INT_MAX){
                n->npc->direction = 3;
                characterMap[n->npc->pos[dim_y]][n->npc->pos[dim_x]] = dummy;
                n->npc->pos[dim_y]++;
              }
              if(n->npc->direction == 0 && rm[n->npc->pos[dim_y]][n->npc->pos[dim_x] - 1] != INT_MAX){
                characterMap[n->npc->pos[dim_y]][n->npc->pos[dim_x]] = dummy;
                n->npc->pos[dim_x]--;
              }else if(rm[n->npc->pos[dim_y]][n->npc->pos[dim_x] + 1] != INT_MAX){
                n->npc->direction = 1;
      ...
      case char_swimmer:
        switch(n->npc->direction){
          case 0:
            if(m->map[n->npc->pos[dim_y]][n->npc->pos[dim_x] - 1] != n->npc->ter){
              n->npc->direction = rand() % 4;
              if((n->npc->direction == 0 || n->npc->direction == 1) && m->map[n->npc->pos[dim_y]][n->npc->pos[dim_x] + 1] == n->npc->ter){
                n->npc->direction = 1;
                characterMap[n->npc->pos[dim_y]][n->npc->pos[dim_x]] = dummy;
              ...
              n->nextTurn += move_cost[3][m->map[n->npc->pos[dim_y]][n->npc->pos[dim_x]]];
              characterMap[n->npc->pos[dim_y]][n->npc->pos[dim_x]] = *n;
            }else{
              characterMap[n->npc->pos[dim_y]][n->npc->pos[dim_x]] = dummy;
              n->npc->pos[dim_x]--;
          ...
          case 1:
            ...
            break;
          case 2:
            if(m->map[n->npc->pos[dim_y] - 1][n->npc->pos[dim_x]] != n->npc->ter){
              n->npc->direction = rand() % 4;
              if((n->npc->direction == 2 || n->npc->direction == 3) && m->map[n->npc->pos[dim_y] + 1][n->npc->pos[dim_x]] == n->npc->ter){
                n->npc->direction = 3;
                characterMap[n->npc->pos[dim_y]][n->npc->pos[dim_x]] = dummy;
        ...
        break;
      default:
        printf("You didnt put stuff into heap right my brotha");
        break;
    }
    n->hn = heap_insert(&h, n);
    printf(" y: %d x: %d", n->npc->pos[dim_y], n->npc->pos[dim_x]);
    print_map(m);
    usleep(125000);
  }
  heap_delete(&h);
...
int main(int argc, char *argv[])
{
  ...
  moveCharacters(world.cur_map, world.hiker_dist, world.rival_dist, numC);
  return 0;
  print_map();
  print_hiker_dist();
  print_rival_dist();
