...
//  boulder,tree,path,mart,center,grass,clearing,mountain,forest,water,gate
  { IM, IM, 10, 10, 10, 20, 10, IM, IM, IM, IM },
  ...
  for(i = 0; i < MAP_Y; ++i){
    for(j = 0; j < MAP_X; ++j){
      if(world.cur_map->cmap[i][j] != NULL){
        ...
        mvprintw(g, 1, "%c: %d %s and %d %s", world.cur_map->cmap[i][j]->symbol, yDist, nors, xDist, eorw);
        ++g;
      }
    }
  }
      ...
      switch(key){
        ...
        case '7':
          if(move_cost[char_pc][world.cur_map->map[c->pos[dim_y] - 1][c->pos[dim_x] - 1]] != INT_MAX){
            if(world.cur_map->cmap[c->pos[dim_y] - 1][c->pos[dim_x] - 1] != NULL){
              battleInterface();
        ...
        case '8':
          if(move_cost[char_pc][world.cur_map->map[c->pos[dim_y] - 1][c->pos[dim_x]]] != INT_MAX){
            if(world.cur_map->cmap[c->pos[dim_y] - 1][c->pos[dim_x]] != NULL){
              battleInterface();
