...
static int32_t move_cost[2][12] = {
  {INT_MAX, INT_MAX, 10, 50, 50, 15, 10, 15, 15, INT_MAX, 0},
  {INT_MAX, INT_MAX, 10, 50, 50, 20, 10, INT_MAX, INT_MAX, INT_MAX, 0},
};
...
{
  static path_t path2[MAP_Y][MAP_X], *p;
  static uint32_t initialized = 0;
  heap_t h;
  uint32_t x, y;
    ...
    case 'n':
      ...
      if(yCurr != 0){
        if(big_map[yCurr - 1][xCurr] != NULL){
          --yCurr;
          print_map(big_map[yCurr][xCurr]);
          dijkstra_path2(big_map[yCurr][xCurr], characterLoc, 0);
          dijkstra_path2(big_map[yCurr][xCurr], characterLoc, 1);
        }else{
