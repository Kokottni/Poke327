...
static void print_map()
{
  ...
  mvprintw(0, 1, "Poke327 Assignment1.05");
  ...
  for (y = 0; y < MAP_Y; y++) {
    for (x = 0; x < MAP_X; x++) {
      if (world.cur_map->cmap[y][x]) {
        ...
        color = COLOR_MAGENTA;
        mvprintw(y + 1, x, "%c", world.cur_map->cmap[y][x]->symbol);
      } else {
        switch (world.cur_map->map[y][x]) {
        case ter_boulder:
          attron(COLOR_PAIR(COLOR_WHITE));
          color = COLOR_WHITE;
          mvprintw(y + 1, x, "%c", BOULDER_SYMBOL);
          break;
        case ter_mountain:
          attron(COLOR_PAIR(COLOR_WHITE));
          color = COLOR_WHITE;
          mvprintw(y + 1, x, "%c", MOUNTAIN_SYMBOL);
          break;
        case ter_tree:
          attron(COLOR_PAIR(COLOR_GREEN));
          color = COLOR_GREEN;
          mvprintw(y + 1, x, "%c", TREE_SYMBOL);
          break;
        case ter_forest:
          attron(COLOR_PAIR(COLOR_GREEN));
          color = COLOR_GREEN;
          mvprintw(y + 1, x, "%c", FOREST_SYMBOL);
          break;
        case ter_path:
          attron(COLOR_PAIR(COLOR_CYAN));
          color = COLOR_CYAN;
          mvprintw(y + 1, x, "%c", PATH_SYMBOL);
          break;
        case ter_gate:
          attron(COLOR_PAIR(COLOR_CYAN));
          color = COLOR_CYAN;
          mvprintw(y + 1, x, "%c", GATE_SYMBOL);
          break;
        case ter_mart:
          attron(COLOR_PAIR(COLOR_RED));
          color = COLOR_RED;
          mvprintw(y + 1, x, "%c", POKEMART_SYMBOL);
          break;
        case ter_center:
          attron(COLOR_PAIR(COLOR_RED));
          color = COLOR_RED;
          mvprintw(y + 1, x, "%c", POKEMON_CENTER_SYMBOL);
          break;
        case ter_grass:
          attron(COLOR_PAIR(COLOR_YELLOW));
          color = COLOR_YELLOW;
          mvprintw(y + 1, x, "%c", TALL_GRASS_SYMBOL);
          break;
        case ter_clearing:
          attron(COLOR_PAIR(COLOR_YELLOW));
          color = COLOR_YELLOW;
          mvprintw(y + 1, x, "%c", SHORT_GRASS_SYMBOL);
          break;
        case ter_water:
          attron(COLOR_PAIR(COLOR_BLUE));
          color = COLOR_BLUE;
          mvprintw(y + 1, x, "%c", WATER_SYMBOL);
          break;
        default:
          attron(COLOR_PAIR(COLOR_MAGENTA));
          color = COLOR_MAGENTA;
          mvprintw(y + 1, x, "%c", ERROR_SYMBOL);
          break;
        }
        attroff(COLOR_PAIR(color));
      }
    }
    mvprintw(23, 1, "Nicholas Kokott <kokottni@iastate.edu>");
    refresh();
  }
}
...
void centerInterface(){
  clear();
  int exit = 0;
  int key;
  mvprintw(0, 1, "Temporary PokeCenter interface, hit left arrow key to exit");
  refresh();
  while(exit == 0){
    key = getch();
    if(key == KEY_LEFT){
...
void invalidInterface(){
  ...
  int exit = 0;
  int key;
  mvprintw(0, 1, "You have invalid coordinates, please try again after hitting the left arrow.");
  refresh();
  while(exit == 0){
    key = getch();
    if(key == KEY_LEFT){
...
void martInterface(){
  ...
  int exit = 0;
  int key;
  mvprintw(0, 1, "Temporary PokeMart interface, hit left arrow key to exit");
  refresh();
  while(exit == 0){
    key = getch();
    if(key == KEY_LEFT){
...
void battleInterface(){
  ...
  int exit = 0;
  int key;
  mvprintw(0, 1, "Temporary battle interface, hit left arrow key to exit");
  refresh();
  while(exit == 0){
    key = getch();
    if(key == KEY_LEFT){
...
void trainerList(){
  ...
  for(i = 0; i < MAP_Y; ++i){
    for(j = 0; j < MAP_X; ++j){
      if(world.cur_map->cmap[i][j] != NULL){
        yDist = i - world.pc.pos[dim_y];
        xDist = j - world.pc.pos[dim_x];
        if(yDist < 0 && xDist < 0){
          yDist = yDist * (-1);
          xDist = xDist * (-1);
          mvprintw(g, 1, "%c: %d %s and %d %s", world.cur_map->cmap[i][j]->symbol, yDist, "North", xDist, "East");
        }else if(yDist > 0 && xDist < 0){
                    xDist = xDist * (-1);
          mvprintw(g, 1, "%c: %d %s and %d %s", world.cur_map->cmap[i][j]->symbol, yDist, "South", xDist, "East");
        }else if(yDist < 0 && xDist > 0){
          yDist = yDist * (-1);
          mvprintw(g, 1, "%c: %d %s and %d %s", world.cur_map->cmap[i][j]->symbol, yDist, "North", xDist, "West");
        }else{
          mvprintw(g, 1, "%c: %d %s and %d %s", world.cur_map->cmap[i][j]->symbol, yDist, "South", xDist, "West");
        }
        ++g;
      }
    }
...
void findPlacement(){
  ...
  for(i = 1; i < 79; ++i){
    for(j = 1; j < 20; ++j){
      if(world.cur_map->map[j][i] == ter_path){
        world.pc.pos[dim_y] = j;
        world.pc.pos[dim_x] = i;
      }
    }
  }
}
      ...
      switch(key){
        ...
        case '7':
          if(move_cost[char_pc][world.cur_map->map[c->pos[dim_y] - 1][c->pos[dim_x] - 1]] != INT_MAX){
            if(world.cur_map->map[c->pos[dim_y] - 1][c->pos[dim_x] - 1] == ter_gate){
              world.cur_map->cmap[c->pos[dim_y]][c->pos[dim_x]] = NULL;
              if(c->pos[dim_x] - 1 == 0){
                world.cur_idx[dim_x]--;               
                new_map();
                world.pc.pos[dim_y] = c->pos[dim_y];
                ...
                world.pc.pos[dim_x] = c->pos[dim_x];          
                changeMap = 1;
              } 
              break;
            }else if(world.cur_map->cmap[c->pos[dim_y] - 1][c->pos[dim_x] - 1] != NULL){
              battleInterface();
              world.cur_map->cmap[c->pos[dim_y]][c->pos[dim_x]] = world.cur_map->cmap[c->pos[dim_y] - 1][c->pos[dim_x] - 1];
              world.cur_map->cmap[c->pos[dim_y]][c->pos[dim_x]]->next_turn = INT_MAX;
            }else{
              world.cur_map->cmap[c->pos[dim_y]][c->pos[dim_x]] = NULL;
            }
            move = 1;
        ...
        case '8':
          if(move_cost[char_pc][world.cur_map->map[c->pos[dim_y] - 1][c->pos[dim_x]]] != INT_MAX){
            if(world.cur_map->map[c->pos[dim_y] - 1][c->pos[dim_x]] == ter_gate){
              world.cur_map->cmap[c->pos[dim_y]][c->pos[dim_x]] = NULL;
              world.cur_idx[dim_y]--;             
              new_map();
              world.pc.pos[dim_y] = 19;
              ...
              changeMap = 1;  
              break;
            }else if(world.cur_map->cmap[c->pos[dim_y] - 1][c->pos[dim_x]] != NULL){
              battleInterface();
              world.cur_map->cmap[c->pos[dim_y]][c->pos[dim_x]] = world.cur_map->cmap[c->pos[dim_y] - 1][c->pos[dim_x]];
              world.cur_map->cmap[c->pos[dim_y]][c->pos[dim_x]]->next_turn = INT_MAX;
            }else{
              world.cur_map->cmap[c->pos[dim_y]][c->pos[dim_x]] = NULL;
            }
        ...
        case '6':
          if(move_cost[char_pc][world.cur_map->map[c->pos[dim_y]][c->pos[dim_x] + 1]] != INT_MAX){
            if(world.cur_map->map[c->pos[dim_y]][c->pos[dim_x] + 1] == ter_gate){
              world.cur_map->cmap[c->pos[dim_y]][c->pos[dim_x]] = NULL;
              world.cur_idx[dim_x]++;    
              new_map(); 
              world.pc.pos[dim_y] = c->pos[dim_y];
              ...
              changeMap = 1;
              break;
            }else if(world.cur_map->cmap[c->pos[dim_y]][c->pos[dim_x] + 1] != NULL){
              battleInterface();
              world.cur_map->cmap[c->pos[dim_y]][c->pos[dim_x]] = world.cur_map->cmap[c->pos[dim_y]][c->pos[dim_x] + 1];
              world.cur_map->cmap[c->pos[dim_y]][c->pos[dim_x]]->next_turn = INT_MAX;
            }else{
              world.cur_map->cmap[c->pos[dim_y]][c->pos[dim_x]] = NULL;
            }
        ...
        case '4':
          if(move_cost[char_pc][world.cur_map->map[c->pos[dim_y]][c->pos[dim_x] - 1]] != INT_MAX){
            ...
            if(world.cur_map->cmap[c->pos[dim_y]][c->pos[dim_x] - 1] != NULL){
              battleInterface();
              world.cur_map->cmap[c->pos[dim_y]][c->pos[dim_x]] = world.cur_map->cmap[c->pos[dim_y]][c->pos[dim_x] - 1];
              world.cur_map->cmap[c->pos[dim_y]][c->pos[dim_x]]->next_turn = INT_MAX;
            }else{
        ...
        case 'F':
          ...
          world.cur_idx[dim_y] = goY + 200;
          world.cur_idx[dim_x] = goX + 200;
          new_map();
          findPlacement();
          changeMap = 1;
          break;
