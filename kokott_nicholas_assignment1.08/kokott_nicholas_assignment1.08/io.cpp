...
int get_level() {
  ...
  int dist = (abs(world.cur_idx[dim_x] - (WORLD_SIZE / 2)) +
...
int get_HP(int id, int lvl) {
  int IV = rand() % 16;
  int base = pokemon_stats[(id * 6) - 5].base_stat;
  int hp = ((((base + IV) * 2) * lvl) / 100) + lvl + 10;
  
  return hp;
}

int get_other(int id, int lvl, int stat_id) {
  int IV = rand() % 16;
  int base, stat;

  switch(stat_id) {
  case 2:
    base = pokemon_stats[(id * 6) - 4].base_stat;
    break;
  case 3:
    base = pokemon_stats[(id * 6) - 3].base_stat;
    break;
  case 4:
    base = pokemon_stats[(id * 6) - 2].base_stat;
    break;
  case 5:
    base = pokemon_stats[(id * 6) - 1].base_stat;
    break;
  case 6:
    base = pokemon_stats[(id * 6)].base_stat;
    break;
  }

  stat = ((((base + IV) * 2) * lvl) / 100) + 5;
...
void get_moves(int id, int lvl) {
  int i, j = 0, mult = 0, random_mov1, random_mov2;

  int *id_arr = (int*)malloc(sizeof(int));
  
  for(i = 0; i < 528239; i++) {
   
    if (pokemon_moves[i].pokemon_id == pokemon[id].species_id) {
      if (pokemon_moves[i].pokemon_move_method_id == 1) {
	if (pokemon_moves[i].level <= lvl) {

	  id_arr[j] = pokemon_moves[i].move_id;
	    
	  j++;

	  id_arr = (int*)realloc(id_arr, (j+1) * sizeof(int));
	}
      }
    }  
  }
  ...
  if (j == 0) {
    mvprintw(23, 10, "NO AVAILABLE MOVE FOR POKE");
    return;
  }
  ...
  mvprintw(7, 3, "MOVESET-> <1>%s", moves[id_arr[random_mov1]].identifier);
    
  if (j == 1 || !mult) {
    return;
  }

  while (random_mov1 == random_mov2 || id_arr[random_mov1] == id_arr[random_mov2]) {
    random_mov2 = rand() % j;
  }
    
  mvprintw(8, 13, "<2>%s", moves[id_arr[random_mov2]].identifier);
...
void pokemon_encounter() {
  if (!(rand() % 10)) {
    ...
    mvprintw(4, 3, "A Wild LVL %d %s(%c) Appears (SHINY?): %c",
	     lvl,  pokemon[id].identifier, get_gender(), get_shiny());
    
    mvprintw(5, 3, "STATS-> HP: %d ATT: %d  DEF: %d",
	     get_HP(id, lvl), get_other(id, lvl, 2), get_other(id, lvl, 3));
    
    mvprintw(6, 3, "STATS-> SP. A: %d SP. D: %d  SPEED: %d",
	     get_other(id, lvl, 4), get_other(id, lvl, 5), get_other(id, lvl, 6));
    
    get_moves(id, lvl);
    
    mvprintw(10, 3, "PRESS 'ESC' KEY TO EXIT");
...
void give_starter(){
  ...
  //io_display();
  mvprintw(3, 2, "%s", "Please choose your starting level 1 pokemon:");
  mvprintw(4, 2, "%s %s", "1. ", p[0].name);
  mvprintw(5, 2, "%s %s", "2. ", p[1].name);
  mvprintw(6, 2, "%s %s", "3. ", p[2].name);
  refresh();
  selected = 0;
  while(selected == 0){
    switch(key = getch()){
...
pokeman makePokemon(){
  ...
  p.id = rand() % 1092 + 1;
  p.level = get_level();
  //p.name = pokemon[p.id].identifier;
  strcpy(p.name, pokemon[p.id].identifier);
  p.gender = get_gender();
  p.shiny = get_shiny();
  p.hp = get_HP(p.id, p.level);
  p.attack = get_other(p.id, p.level, 2);
  p.defense = get_other(p.id, p.level, 3);
  p.speed = get_other(p.id, p.level, 6);
  p.specialAttack = get_other(p.id, p.level, 4);
  p.specialDefense = get_other(p.id, p.level, 5);
  return p;
}
...
void io_battle(Character *aggressor, Character *defender)
{
  ...
  io_display();
  clear();
    mvprintw(0, 0, "Here are my pokemon please hit esc to dismiss me as useless");
  if(aggressor->symbol == '@'){
    Npc *pokes = dynamic_cast<Npc *>(defender);
    while(pokes->p[i].id != -1 && i < 6){
      mvprintw(i + 2, 0, "%s, Level: %d, HP: %d, Attack: %d, Defense: %d", pokes->p[i].name, pokes->p[i].level, pokes->p[i].hp, pokes->p[i].attack, pokes->p[i].defense);
      ++i;
    }
  }else{
    Npc *pokes = dynamic_cast<Npc *>(aggressor);
    while(pokes->p[i].id != -1 && i < 6){
      mvprintw(i + 2, 0, "%s, Level: %d, HP: %d, Attack: %d, Defense: %d", pokes->p[i].name, pokes->p[i].level, pokes->p[i].hp, pokes->p[i].attack, pokes->p[i].defense);
      ++i;
    }
